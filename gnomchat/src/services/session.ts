...
// Ported from the website's src/services/vocechatSession.ts. Same token lifecycle
// (renew with a margin, single-flight refresh), but session state lives in the
...
export async function ensureFreshVoceChatToken(force = false): Promise<string | null> {
  ...
  if (!session.refreshToken || session.expiresAt === null) return session.token;
  if (!force && Date.now() < session.expiresAt - REFRESH_MARGIN_MS) return session.token;
...
export async function refreshVoceChatToken(force = false): Promise<boolean> {
  ...
  refreshPromise = (async () => {
    try {
      ...
      if (!res.ok) {
        if (res.status === 401 || res.status === 403) clearVoceChatSession();
        return false;
      }

      const payload = (await res.json()) as TokenPayload;
      const currentUser = useAuthStore.getState().user;
      if (!currentUser?.uid || !currentUser?.name) return false;
