...
export const config = {
  ...
  bryggeriUrl: resolveBryggeriUrl(window.__APP_ENV__?.BRYGGERI_URL),
...
function resolveBryggeriUrl(value: string | undefined) {
  const trimmed = value?.trim();
  return trimmed ? trimmed : defaultBryggeriUrl;
}
